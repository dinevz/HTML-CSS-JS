import {getData} from './getIcons.js';

let url = 'https://api.coingecko.com/api/v3';
let moves = 0;
let arr = [];

let icons = await getData(url);
icons.forEach(element => {
    if(element.id != 'tether' && element.id != 'usd-coin' && element.id != 'binance-usd' && element.id != 'dai' && element.id != 'terrausd') {
...
const gameLogic = (e, cards) => {
    ...
    //         if(turned.length < 2 && e.currentTarget.firstChild.style.visibility != 'hidden'){
    //             turnFront(e);
    //             turned = document.querySelectorAll('.turned');
    //         }
